...
async def send_doc(client, message):
    ...
    used_date = user_deta.get("date", None)
    ...
    if remain < int(file.file_size):
        await message.reply_text(f"100% Of Daily {humanize(limit)} Data Quota Exhausted.\n\n<b>File Size Detected :</b> {humanize(file.file_size)}\n<b>Used Daily Limit :</b> {humanize(used)}\n\nYou Have Only <b>{humanize(remain)}</b> Left On Your Account.\n\nIf U Want To Rename Large File Upgrade Your Plan", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("💳 Upgrade", callback_data="my_pl_call")]]))
        return
    if value < file.file_size:
        ...
        filesize = humanize(file.file_size)
        fileid = file.file_id
        total_rename(int(botid), prrename)
        total_size(int(botid), prsize, file.file_size)
